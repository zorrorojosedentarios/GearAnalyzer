...
-- Hardcoded item classification tables (constantes para evitar recreación en EvaluateItem)
local HARDCODED_TANK_ITEMS = {
...
local HARDCODED_HEAL_ITEMS = {
    [50359]=true, [50366]=true, [50339]=true, [50346]=true, [50358]=true, [50357]=true,
    [47041]=true, [47059]=true, [47271]=true, [47432]=true,
}

local INST_ABBREV = { ["Icecrown Citadel"] = "ICC", ["The Ruby Sanctum"] = "RS", ["Trial of the Crusader"] = "TotC" }
...
function GearAnalyzer:GetSourceFromPT3(itemID)
    ...
    for _, v in pairs(sets) do
        ...
        if set == "RaidLoot" then
            ...
            instance = INST_ABBREV[instance] or instance
        ...
        for zone, bosses in pairs(_G.lootTable) do
            for boss, items in pairs(bosses) do
                for _, itemId in ipairs(items) do
                    if itemId == id then
                        local cleanZone = INST_ABBREV[zone] or zone
...
-- Cache de clase del jugador para GetSlotKey / GetINVTYPE
local PLAYER_CLASS_FILENAME = select(2, UnitClass("player"))

-- Mapas estáticos pre-construidos (nunca cambian)
local SLOT_KEY_MAP_BY_ID = {
    ...
    [18] = (PLAYER_CLASS_FILENAME == "HUNTER") and "ranged" or "idol",
}

local SLOT_KEY_MAP_BY_NAME = {
...
local INVTYPE_MAP_BY_ID = {
    ...
    [18] = (PLAYER_CLASS_FILENAME == "HUNTER" or PLAYER_CLASS_FILENAME == "ROGUE" or PLAYER_CLASS_FILENAME == "WARRIOR") and "INVTYPE_RANGED" or "INVTYPE_RELIC",
}

local INVTYPE_MAP_BY_NAME = {
...
function GearAnalyzer:GetSlotKey(slotName, slotID)
    if slotID and SLOT_KEY_MAP_BY_ID[slotID] then
        return SLOT_KEY_MAP_BY_ID[slotID]
    end
    return SLOT_KEY_MAP_BY_NAME[slotName]
end

function GearAnalyzer:GetINVTYPE(slotName, slotID)
    if slotID and INVTYPE_MAP_BY_ID[slotID] then
        return INVTYPE_MAP_BY_ID[slotID]
    end
    return INVTYPE_MAP_BY_NAME[slotName]
end
...
function GearAnalyzer:AnalyzeEquipment(ignoreForced)
    ...
    -- [CORRECCIÓN CAMPOS STALE] Limpiar campos dependientes de la especialización
    local prefix = ignoreForced and "" or "guide_"
    if self.scannedEquipment then
        for _, d in ipairs(self.scannedEquipment) do
            d[prefix.."isIncorrect"] = nil
            d[prefix.."isBIS"] = nil
            d[prefix.."isAlternative"] = nil
            d[prefix.."rank"] = nil
            d[prefix.."nextUpgradeID"] = nil
            d[prefix.."enchStatus"] = nil
            d[prefix.."enchTooltip"] = nil
            d[prefix.."isCorrect"] = nil
            
            if ignoreForced then
                d.isTier10Normal = nil
                d.gemStatus = nil
                d.gemTooltip = nil
                d.socketBonusText = nil
                d.bonusStat = nil
                d.bonusVal = nil
                d.socketSlotsAvailable = nil
                d.socketAnalysis = nil
                d.betterInBags = nil
                d.betterInBank = nil
                d.betterItemLink = nil
                d.betterItemBag = nil
                d.betterItemSlot = nil
                d.setInfo = nil
            end
        end
    end
    ...
    local devLog = {}
    local devMode = GearAnalyzer.db.profile.settings.devMode
        ...
        if d.itemLink and d.gems then
            for i, gid in ipairs(d.gems) do
                if gid and gid > 0 then
                    ...
                    if devMode then
                        table.insert(devLog, string.format("[%s] ID:%d Color:%s", gName, gid, tostring(gColor)))
                    end
    ...
    if devMode and colorCounts.metaLink then
        self.lastGemCountLog = table.concat(devLog, "\n")
    end
    ...
    -- [RECOMENDACIONES DE INVENTARIO] (fallback standalone si no existe Recommender)
    if ignoreForced then
        ...
        if Rec then
            ...
            for _, d in ipairs(self.scannedEquipment) do
                ...
                local top6 = slotKey and top6Data and top6Data[slotKey]
                local pairKey = checkPairs[slotKey]
                local best = self:GetBestItemInBags(d.slotName, d.slotID, d.itemLink, top6)
                if best then
                    d.betterInBags = not best.isBank
                    d.betterInBank = best.isBank
                    d.betterItemLink = best.link
...
function GearAnalyzer:AnalyzeItemEnchant(d, specEnchants, ignoreForced)
    ...
    if enchantID and enchantID > 0 then
        ...
        if isMatch then
            ...
            local guideParts = {}
            for _, r in ipairs(recs) do
                table.insert(guideParts, r.name .. " (ID: "..tostring(r.checkID)..")")
            end
            local guideNames = table.concat(guideParts, " / ")
...
function GearAnalyzer:EvaluateItem(link)
    ...
    local currentSpec = self.temp_currentSpec or self:GetCurrentSpecEnhanced()
    local class = self.temp_currentClass or self:GetClassToken()
    local normSpec = self:NormalizeSpecName(currentSpec)
    local cacheKey = link .. "|" .. normSpec
    if self.evaluationCache[cacheKey] then local c = self.evaluationCache[cacheKey]; return c.category, c.iLevel end
    ...
    if data.quality < 2 then return "NEUTRAL", data.iLevel end
    ...
    if itemID then
        isTankItem = HARDCODED_TANK_ITEMS[itemID]
        isMeleeItem = HARDCODED_MELEE_ITEMS[itemID]
        isCasterItem = HARDCODED_CASTER_ITEMS[itemID]
        isHealItem = HARDCODED_HEAL_ITEMS[itemID]
    end
    ...
    if isMainArmor and (class == "DEATHKNIGHT" or class == "WARRIOR" or class == "PALADIN") then
        ...
        if isWrongArmor or itemIDNum == 50449 then self.evaluationCache[cacheKey] = { category = "INCORRECT", iLevel = data.iLevel }; return "INCORRECT", data.iLevel end
        if not (sub:find("placa") or sub:find("plate")) and sub ~= "" then self.evaluationCache[cacheKey] = { category = "INCORRECT", iLevel = data.iLevel }; return "INCORRECT", data.iLevel end
    end
    ...
    self.evaluationCache[cacheKey] = { category = category, iLevel = data.iLevel }
...
function GearAnalyzer:GetGemWeights(class, spec)
    ...
    local normSpec = self:NormalizeSpecName(spec)
    local isTank = normSpec:find("Tank") or normSpec:find("Bear") or normSpec == "Protection"
    if class == "DEATHKNIGHT" then
        if isTank then weights.STA = 3.0; weights.STR = 1.0; weights.ARP = 0.5; weights.CRIT = 0.5; weights.HASTE = 0.5; weights.AGI = 0.8; weights.AP = 0.5; weights.DEFENSE = 2.0; weights.DODGE = 1.5; weights.PARRY = 1.5; weights.BLOCK = 1.0
    ...
    elseif class == "ROGUE" then
        if normSpec == "Assassination" then weights.AP = 2.0; weights.AGI = 1.8; weights.HASTE = 1.5; weights.CRIT = 1.2
        else weights.ARP = 2.5; weights.AGI = 2.0; weights.AP = 1.8; weights.CRIT = 1.4; weights.HASTE = 1.3 end
    elseif class == "HUNTER" then
        if normSpec == "Marksmanship" then weights.ARP = 2.4; weights.AGI = 2.0; weights.CRIT = 1.6; weights.AP = 1.0
        else weights.AGI = 2.2; weights.CRIT = 1.8; weights.HASTE = 1.4; weights.AP = 1.0 end
    elseif normSpec == "Shadow" then
        weights.SP = 1.0; weights.HASTE = 0.98; weights.CRIT = 0.76; weights.SPIRIT = 0.2
    elseif isTank then
        weights.STA = 3.0; weights.STR = 1.5; weights.AP = 0.5; weights.ARP = 0.3; weights.DEFENSE = 2.0; weights.DODGE = 1.5; weights.PARRY = 1.5; weights.BLOCK = 1.0
    else
        weights.STR = 2.0; weights.AGI = 2.0; weights.SP = 1.0; weights.STA = 1.5; weights.INT = 1.0
    end
    return weights
end
...
function GearAnalyzer:CalculateItemScore(link, class, spec)
    ...
    self.scoreCache[class][spec][link] = finalScore
    self._cacheCounts.score = (self._cacheCounts.score or 0) + 1
    if self._cacheCounts.score > 500 then
        self.scoreCache = {}

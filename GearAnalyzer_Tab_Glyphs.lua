...
function Tab:Update(page, ignoreForced)
    ...
    local glyphParts = { spec }
    for i = 1, 6 do
        local _, _, id = GetGlyphSocketInfo(i)
        glyphParts[i + 1] = id or 0
    end
    local glyphHash = table.concat(glyphParts)
    ...
    if not self.container then
        ...
        scrollLeft:SetPoint("BOTTOMRIGHT", -9, 10)

        local contentLeft = CreateFrame("Frame", nil, scrollLeft)
        contentLeft:SetWidth(355)
        ...
        rightList:SetPoint("BOTTOMRIGHT", 0, 10)

...
function GearAnalyzer:ToggleDevMode()
    ...
    if settings.devMode then
        ...
        if ItemRefTooltip and not self._gaHookedRef then
            ItemRefTooltip:HookScript("OnTooltipSetItem", function(tooltip)
                self:OnTooltipSetItem(tooltip)
            end)
            self._gaHookedRef = true
        end
        if ShoppingTooltip1 and not self._gaHookedShop then
            ShoppingTooltip1:HookScript("OnTooltipSetItem", function(tooltip)
                self:OnTooltipSetItem(tooltip)
            end)
            self._gaHookedShop = true
        end
    else
        if self:IsHooked(GameTooltip, "OnTooltipSetItem") then
            self:Unhook(GameTooltip, "OnTooltipSetItem")
        end
        self._gaHookedRef = nil
        self._gaHookedShop = nil
    end

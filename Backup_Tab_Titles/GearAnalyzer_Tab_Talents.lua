...
function Tab:Update(page, ignoreForced)
    ...
    if not self.container then
        ...
        header:SetSize(1005, 32); header:SetPoint("TOPLEFT", 0, 5)
        ...
        treeArea:SetSize(1005, 435); treeArea:SetPoint("TOPLEFT", header, "BOTTOMLEFT", 0, -5)
        self.treeArea = treeArea

        self.panes = {}
        for i = 1, 3 do
            local p = CreateFrame("Frame", nil, treeArea)
            p:SetSize(332, 435); p:SetPoint("LEFT", (i-1)*335, 0)
            ...
            p.name = p:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge"); p.name:SetPoint("TOP", 0, -10); p.name:SetTextColor(1, 0.8, 0)
        ...
        footer:SetSize(1005, 40); footer:SetPoint("TOPLEFT", treeArea, "BOTTOMLEFT", 0, -5)
...
function Tab:RenderTreeGrid(pane, tabIndex, guideData, isSimulated, layoutDB, isPet)
    ...
    local spacingH, spacingV = (isPet and 85 or 35), (isPet and 65 or 35)
    local xOff, yOff = (isPet and 95 or 118), (isPet and 75 or 47)

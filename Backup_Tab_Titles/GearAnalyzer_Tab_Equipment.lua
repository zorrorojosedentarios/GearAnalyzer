...
function Tab:Update(page, ignoreForced)
    ...
    if not header.hEquip then
        ...
        header.hEquip:SetText(L["ANALYSIS"] .. ": " .. classLoc .. " " .. specLabel)
        header.hEquip:SetTextColor(1, 0.82, 0)

        header.hRecomendado = header:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        header.hRecomendado:SetPoint("RIGHT", header, "RIGHT", -265, 0)
        header.hRecomendado:SetText("RECOMENDADO")
...
function Tab:GetOrCreateRow(i, parent)
    ...
    nextIcon:SetSize(32, 32) -- Tamaño aumentado (antes 28)
    ...
    bBtn:SetSize(75, 22)
    bBtn:SetPoint("RIGHT", row, "RIGHT", -260, 0)
    ...
    divider:SetPoint("RIGHT", nextIcon, "LEFT", -15, 0)

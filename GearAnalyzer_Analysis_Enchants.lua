...
function Enchants:AnalyzeItem(itemData, ignoreForced)
    ...
    -- 3. Obtener recomendación de la Spec
    local State = GearAnalyzer:GetModule("State")
    local classToken, specKey = State:GetPlayerState()
    ...
    local specEnchants = classData.Enchants[specKey] or classData.Enchants[specKey.." DPS"] or classData.Enchants[specKey.." Tank"]
    ...
    for _, id in ipairs(recList) do
        ...
        if d1 and d2 and d1.name and d2.name then
            local function n(s) return s:lower():gsub("[^a-z]", "") end

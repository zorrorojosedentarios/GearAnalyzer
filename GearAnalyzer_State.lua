...
function State:Refresh()
    -- UnitClass("player") no cambia en sesión; cachear tras primera llamada
    if not cachedClassToken then
        _, cachedClassToken = UnitClass("player")
    end
    
    -- Usamos la función nativa de GearAnalyzer para obtener la rama actual
    local specName = GearAnalyzer:GetCurrentSpec() or "None"
    
    if DICTIONARY[cachedClassToken] then
        playerSpecKey = DICTIONARY[cachedClassToken].specs[specName] or specName
    else
...
function State:GetPlayerState()
    self:Refresh()
    local dbClass = DICTIONARY[cachedClassToken] and DICTIONARY[cachedClassToken].dbKey or cachedClassToken
    return cachedClassToken, playerSpecKey, dbClass
end

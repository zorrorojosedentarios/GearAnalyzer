...
-- Cache de UnitClass("player") (nunca cambia en sesión)
local playerClassToken = nil
local function GetPlayerClassToken()
    if not playerClassToken then
        _, playerClassToken = UnitClass("player")
    end
...
function GearAnalyzer:GetCurrentSpec(ignoreForced)
    ...
    if self.db and self.db.profile and self.db.profile.settings and self.db.profile.settings.devMode then
        print("|cff3fc7eb[GearAnalyzer Debug]|r Talents: tab1 (Bal) =", tab1, "tab2 (Fer) =", tab2, "tab3 (Resto) =", tab3)
    end
    ...
    if class == "DRUID" then
        ...
        local isBear = false
        local numTalents = GetNumTalents(2) or 0
        for t = 1, numTalents do
            local name, icon, _, _, rank = GetTalentInfo(2, t, false, false, activeGroup)
            if name then
                local lname = name:lower()
                local licon = (icon or ""):lower()
                if licon:find("undyingstrength") or licon:find("invisibilty") or 
                   lname:find("survival of the") or lname:find("supervivencia del") or 
                   lname:find("thick") or lname:find("piel de cuero") then
    ...
    elseif class == "DEATHKNIGHT" then
        ...
        if tab1 > tab2 and tab1 > tab3 then 
            ...
            local numTalents = GetNumTalents(1) or 0
            for t = 1, numTalents do
                local name, icon, _, _, rank = GetTalentInfo(1, t, false, false, activeGroup)
                if name then
                    local lname = name:lower()
                    local licon = (icon or ""):lower()
                    if licon:find("vampiricblood") or licon:find("ancestralawakening") or 
                       lname:find("vampiric") or lname:find("sangre vamp") then
                        if rank and rank > 0 then
                            isDKTank = true
                            break
                        end
            ...
            if not isDKTank then
                isDKTank = isTank or (GetCombatRating(CR_DEFENSE_SKILL or 2) or 0) > 350
            end
...
function GearAnalyzer:GetCurrentSpecEnhanced(ignoreForced)
    ...
    if self.db and self.db.profile and self.db.profile.settings and self.db.profile.settings.devMode then
        print("|cff3fc7eb[GearAnalyzer Debug]|r GetCurrentSpecEnhanced(ignoreForced=" .. tostring(ignoreForced) .. ") => " .. tostring(result) .. " (talentSpec=" .. tostring(talentSpec) .. ", fallbackSpec=" .. tostring(fallbackSpec) .. ")")
    end
    return result
end
...
function GearAnalyzer:NormalizeSpecName(spec)
    ...
    if s:find("sangre") or s:find("blood") then 
        ...
        if s:find("tanque") or s:find("tank") or s:find("tanq") or self:IsTank() then return "Blood Tank" end
        return "Blood DPS"
    end
    ...
    if s:find("feral") then
        ...
        if s:find("tanque") or s:find("tank") or s:find("tanq") or s:find("oso") or s:find("bear") or self:IsTank() then return "Feral Bear" end
        return "Feral Cat"
    end
    if s:find("equilibrio") or s:find("balance") then return "Balance" end
    if s:find("restauraci") or s:find("resto") then return "Restoration" end
...
function GearAnalyzer:IsTank(ignoreForced)
    ...
    if defense > 350 or (defenseSkill and defenseSkill > 500) then return true end 
...
function GearAnalyzer:GetLocalizedClassName(ignoreForced)
    ...
    if not classTag or classTag == "AUTO" then classTag = GetPlayerClassToken() end

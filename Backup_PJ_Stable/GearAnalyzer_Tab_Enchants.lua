-- ============================================================
-- GearAnalyzer Tab: Encantamientos (Sincronizado y Gráfico)
-- ============================================================
local GearAnalyzer = LibStub("AceAddon-3.0"):GetAddon("GearAnalyzer")
local L = LibStub("AceLocale-3.0"):GetLocale("GearAnalyzer")
local Tab = GearAnalyzer:NewModule("TabEncantamientos", "AceTimer-3.0")
...
function Tab:Update(page, ignoreForced)
    ...
    -- 1. CREACIÓN ÚNICA DE ELEMENTOS (CONTENEDOR, SCROLL Y TÍTULO)
    if not self.container then
        local container = CreateFrame("Frame", nil, page)
        container:SetAllPoints()
        self.container = container

        -- Crear el título primero
        ...
        local scroll = CreateFrame("ScrollFrame", "GAEnchantsScroll", container, "UIPanelScrollFrameTemplate")
        scroll:SetPoint("TOPLEFT", 5, -40)
        scroll:SetPoint("BOTTOMRIGHT", -25, 10)
        self.scroll = scroll

        local content = CreateFrame("Frame", nil, scroll)
        local w = container:GetWidth()
        if not w or w <= 0 then w = 450 end
    ...
    self.titleString:SetText((L["ENCHANT_GUIDE"] or "Guía de Encantamientos") .. " - " .. classLoc .. " " .. specLabel)
    
    for _, r in ipairs(self.rows) do r:Hide() end

    -- Obtener data de la nueva DB dinámicamente
    ...
    -- Validar existencia de datos para la especialidad actual
    if not specEnchants then
        ...
        local errText = L["NO_ENCHANT_GUIDE_FOUND"] or "|cffffb300No hay guías de encantamiento registradas para:|r"
        self.errorMsg:SetText(errText .. "\n|cffffffff" .. class .. "_" .. (activeSpec or "Desconocida") .. "|r\n\n|cffaaaaaa(Especialidad: " .. (activeSpec or "Desconocida") .. ")|r")
        self.errorMsg:Show()
        return 
    ...
    local SLOT_LIST = {
        { id = "head",      name = L["SLOT_HEAD"] or "Cabeza" },
        { id = "shoulders", name = L["SLOT_SHOULDERS"] or "Hombros" },
        { id = "back",      name = L["SLOT_BACK"] or "Espalda" },
        { id = "chest",     name = L["SLOT_CHEST"] or "Pecho" },
        { id = "wrists",    name = L["SLOT_WRISTS"] or "Muñecas" },
        { id = "hands",     name = L["SLOT_HANDS"] or "Manos" },
        { id = "legs",      name = L["SLOT_LEGS"] or "Piernas" },
        { id = "feet",      name = L["SLOT_FEET"] or "Pies" },
        { id = "waist",     name = L["SLOT_WAIST"] or "Cintura" },
        { id = "weapon",    name = L["SLOT_WEAPON"] or "Arma Principal" },
        { id = "offhand",   name = L["SLOT_OFFHAND"] or "Arma Secundaria / Escudo" },
        { id = "ranged",    name = L["SLOT_RANGED"] or "Rango" },
    }

    -- Asegurar que el análisis esté fresco
    ...
    for _, slotInfo in ipairs(SLOT_LIST) do
        ...
        if type(rawEnchantID) == "table" then
            if #rawEnchantID > 0 then isValue = true end
        elseif type(rawEnchantID) == "number" and rawEnchantID > 0 then
            isValue = true
        end

        if isValue then
            local displayID = type(rawEnchantID) == "table" and rawEnchantID[1] or rawEnchantID
            ...
            local enchant = {
                ...
                name = map and map.name or ("|cffaaaaaa[ID:" .. displayID .. "]|r"),
            ...
            if GearAnalyzer.scannedEquipment then
                for _, d in ipairs(GearAnalyzer.scannedEquipment) do
                    if GearAnalyzer:GetSlotKey(d.slotName, d.slotID) == slotInfo.id then
                        ...
                        -- Caso especial Cintura (Hebilla Eterna)
                        if slotInfo.id == "waist" and currentID == 0 then
                            local p = { strsplit(":", d.itemLink or "") }
                            if p[4] and p[4] ~= "0" and p[4] ~= "" then currentID = 3731 end
                        end
                        break
                    end
                end
            ...
            local statsText = enchant.stats or ""
            local originText = (enchant.origin ~= "") and (" |cffaaaaaa[" .. enchant.origin .. "]|r") or ""
            ...
            -- MOSTRAR COMPARACIÓN DE IDs (Server vs Equipado)
            local devInfo = ""
            if isDev then
            ...
            for _, d in ipairs(GearAnalyzer.scannedEquipment or {}) do
                ...
                if dataSlotKey == slotInfo.id then
                    local statusProperty = (ignoreForced and "" or "guide_") .. "enchStatus"
                    local enchStatus = d[statusProperty]
                    if enchStatus == 2 then
                        statusIcon = "Interface\\Buttons\\UI-CheckBox-Check"
                        statusColor = {0, 1, 0}
                    elseif enchStatus == 3 then
                        statusIcon = "Interface\\Buttons\\UI-CheckBox-Check"
                        statusColor = {1, 0.65, 0} -- Naranja (Profesión)
                    elseif enchStatus == 1 then
            ...
            if isDev then
                ...
                row.slotText:SetWidth(700) -- Aumentamos el ancho para evitar saltos de línea innecesarios
                
                row.srcText:ClearAllPoints()
                row.srcText:SetPoint("LEFT", row.icon, "RIGHT", 10, -vOffset)
                row.srcText:SetJustifyH("LEFT")
                row.srcText:SetWidth(700)
                row.srcText:Show()
    ...
    if not self.legend then
        ...
        text:SetText(
            "|cff00ff00(Palomita Verde)|r: Encantamiento correcto\n" ..
...
function Tab:GetOrCreateRow(i)
    ...
    -- Shift+Click para linkear en chat / Ctrl+Click para ver
        ...
        if button == "LeftButton" then
            ...
            if self.enchantID and self.enchantID > 0 then
                if GetSpellInfo(self.enchantID) then
                    ...
                    link = itemLink or ("item:" .. self.enchantID)
                end
            end
            if link then

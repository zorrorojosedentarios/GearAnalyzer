...
function Tab:RefreshTalentData()
    ...
    for i = 1, 3 do
        ...
        if self.isGuideMode and exportCode then
            ...
            for talentIdx = 1, branchSize do
                local byte = string.byte(exportCode, offset + talentIdx)
                branchData[talentIdx] = byte and (byte - 48) or 0
            end
...
function Tab:RefreshPetTalentData()
    ...
    if self.isGuideMode and petCode then
        local petLen = string.len(petCode)
        for i = 1, petLen do
            pData[i] = (string.byte(petCode, i) or 48) - 48
        end
...
function Tab:RenderTreeGrid(pane, tabIndex, guideData, isSimulated, layoutDB, isPet)
    ...
    -- Clean up stale icons beyond current talent count
    for k in pairs(pane.talentIcons) do
        if type(k) == "number" and k > numTalents then
            pane.talentIcons[k] = nil
        end
    end
end
